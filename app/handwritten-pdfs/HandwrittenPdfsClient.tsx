// ✅ FILE PATH: app/handwritten-pdfs/HandwrittenPdfsClient.tsx
"use client";
...
// ✅ Category name must match DB category exactly
const HANDWRITTEN = "Handwritten PDFs";
...
export default function HandwrittenPdfsClient() {
  ...
  const [selectedCat, setSelectedCat] = useState<string[]>(
    urlCategory ? safeSplitComma(urlCategory) : [HANDWRITTEN]
  );
  ...
  // ✅ keep this page URL only when category stays Handwritten PDFs-only
  const syncHandwrittenUrl = (
    nextCourse: string,
    nextSession: string,
    nextSearch: string
  ) => {
    ...
    params.set("category", HANDWRITTEN);
    ...
    const qs = params.toString();
    router.replace(`/handwritten-pdfs${qs ? `?${qs}` : ""}`, { scroll: false });
  };

  // ✅ toggle category rule:
  // If user selects multiple categories OR any non-Handwritten => redirect to /products with same filters
  const handleToggleCategory = (cat: string) => {
    const current = Array.isArray(selectedCat) ? selectedCat : [HANDWRITTEN];
    const next = current.includes(cat)
      ? current.filter((c) => c !== cat)
      : [...current, cat];
    const normalized = next.length === 0 ? [HANDWRITTEN] : next;

    const hasOther = normalized.some((c) => c !== HANDWRITTEN);
    ...
    setSelectedCat([HANDWRITTEN]);
    syncHandwrittenUrl(selectedCourse, selectedSession, search);
  };
  ...
  useEffect(() => {
    ...
    const hasOther = c.some((x) => x !== HANDWRITTEN);
  ...
  useEffect(() => {
    const t = setTimeout(() => {
      ...
      if (q === search) return;
      setSearch(q);
      syncHandwrittenUrl(selectedCourse, selectedSession, q);
    }, 500);
  ...
  const breadcrumbText = useMemo(() => HANDWRITTEN, []);
    ...
    <main className="min-h-screen font-sans text-slate-800 bg-white">
      ...
      <div className="bg-white border-b border-gray-100">
        <div className="max-w-[1600px] mx-auto px-4 py-3 text-[13px] text-gray-500 flex items-center gap-2 overflow-x-auto whitespace-nowrap">
          ...
          <Link href="/handwritten-pdfs" className="hover:text-blue-700 font-semibold">Handwritten PDFs</Link>
          <ChevronRight size={14} className="text-gray-300" />
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="relative max-w-[1600px] mx-auto px-4 py-7 md:py-12">
          <div className="flex flex-col lg:flex-row lg:items-center gap-6 lg:gap-10">
            <div className="flex-1 min-w-0">
              <div className="inline-flex items-center gap-2 rounded-full border border-blue-200 bg-blue-50 px-3 py-1 text-[11px] font-extrabold text-blue-700">
                <Sparkles size={14} /> Premium Handwritten Notes
              </div>
              ...
              <p className="mt-2 text-sm md:text-lg font-medium text-slate-600 max-w-3xl">
                Select course & session to find the exact handwritten notes. This page is dedicated to Handwritten PDFs only.
              </p>
              ...
              <div className="mt-5 md:hidden space-y-3">
                ...
                <div className="w-full rounded-2xl border border-gray-200 bg-white/90 backdrop-blur shadow-sm p-3">
                  <div className="text-[11px] font-extrabold text-slate-700 uppercase mb-2">Search handwritten PDFs</div>
                  ...
                  <div className="mt-2 text-[11px] text-slate-500 font-semibold">
                    Filters help you find the exact course & session faster.
                  </div>
                </div>
              </div>
            </div>
                ...
                <div className="rounded-2xl border border-gray-200 bg-white/90 backdrop-blur shadow-sm p-4">
                  <div className="flex items-center gap-2">
                    ...
                    <div className="text-xs">
                      <div className="font-extrabold text-slate-900">Clear Handwriting</div>
                      <div className="font-bold text-gray-500">Readable notes</div>
                    </div>
                  </div>
                </div>
                <div className="rounded-2xl border border-gray-200 bg-white/90 backdrop-blur shadow-sm p-4">
              ...
              <div className="rounded-2xl border border-gray-200 bg-white shadow-sm overflow-hidden">
                <div className="p-4 bg-gradient-to-r from-blue-700 via-indigo-700 to-cyan-600 isp-shimmer text-white">
                  ...
                  <div className="mt-1 text-xs font-bold opacity-90">
                    Choose Course + Session to find exact handwritten notes.
                  </div>
                </div>
                <div className="p-5">
                  <div className="text-xs font-extrabold text-slate-900">Best way to find your notes</div>
          ...
          {/* ✅ Desktop-only SEO ribbon (mobile hidden) */}
          <div className="hidden md:block mt-6 rounded-2xl border border-gray-200 bg-white/80 backdrop-blur shadow-sm p-5">
            <div className="text-base font-extrabold text-slate-900">
              Download Handwritten PDFs (Course & Session-wise)
            </div>
            <div className="mt-1 text-sm font-semibold text-slate-600 leading-relaxed">
              Choose Course & Session for best matching handwritten notes. If you select multiple categories, you’ll be redirected to All Products automatically.
            </div>
            <div className="mt-3">
              <Link
                href="/products"
        ...
        <div className="max-w-[1600px] mx-auto px-4">
          <div className="flex flex-col lg:flex-row gap-8 items-start relative">
            ...
            <div className="flex-1 w-full min-w-0">
              ...
              <div className="mt-5">
                <ProductGrid selectedCat={[HANDWRITTEN]} onMeta={setMeta} />
              </div>
      ...
      <section className="bg-white border-t border-gray-100">
          ...
          <div className="rounded-2xl border border-gray-200 bg-gray-50 p-5 md:p-6 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
            <div className="min-w-0">
              ...
              <div className="mt-1 text-sm font-semibold text-slate-600">
                Explore all categories in one place (Ebooks, Projects, Solved Assignments, PYQ, Combo, etc).
              </div>
            </div>
        ...
        <div className="max-w-[1600px] mx-auto px-4 py-12 md:py-16">
          ...
          <div className="grid md:grid-cols-3 gap-4">
            {[
              { t: "Handwritten & Readable", d: "Clean notes with exam-focused content." },
