...
async function getInitialHandwrittenPdfsData(input: {
  course?: string;
  session?: string;
  language?: string;
  search?: string;
...
export async function generateMetadata({ searchParams }: PageProps): Promise<Metadata> {
  ...
  const baseTitle = "Handwritten PDFs";
      ...
      images: [
        {
          ...
          width: 1200,
          height: 630,
          alt: "IGNOU Handwritten PDFs - IGNOU Students Portal",
        },
      ],
    },
    twitter: {
...
export default async function Page({ searchParams }: PageProps) {
  ...
  const { products, meta } = await getInitialHandwrittenPdfsData({
  ...
  const initialQueryKey = buildHandwrittenPdfsQueryKey({
    course: initialCourseParam,
    session: initialSessionParam,
    language: initialLanguageParam,
    search: initialSearchParam,
