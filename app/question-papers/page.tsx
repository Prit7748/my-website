// ✅ FILE PATH: app/question-papers/page.tsx
"use client";
...
export default function QuestionPapersPage() {
   ...
   const [selectedCat, setSelectedCat] = useState<string[]>(
      urlCategory ? safeSplitComma(urlCategory) : [PYQ]
   );
   ...
   // ✅ keep this page URL only when category stays PYQ-only
   const syncPyqUrl = (nextCourse: string, nextSession: string, nextSearch: string) => {
      const params = new URLSearchParams(searchParams.toString());

      params.set("category", PYQ);
      ...
      const qs = params.toString();
      router.replace(`/question-papers${qs ? `?${qs}` : ""}`, { scroll: false });
   };

   // ✅ toggle category rule:
   // If user selects multiple categories OR any non-PYQ => redirect to /products with same filters
   const handleToggleCategory = (cat: string) => {
      const current = Array.isArray(selectedCat) ? selectedCat : [PYQ];
      const next = current.includes(cat) ? current.filter((c) => c !== cat) : [...current, cat];
      const normalized = next.length === 0 ? [PYQ] : next;

      const hasOther = normalized.some((c) => c !== PYQ);
      ...
      setSelectedCat([PYQ]);
      syncPyqUrl(selectedCourse, selectedSession, search);
   };
   ...
   useEffect(() => {
      const t = setTimeout(() => {
         ...
         if (q === search) return;
         setSearch(q);
         syncPyqUrl(selectedCourse, selectedSession, q);
      }, 500);
   ...
   const breadcrumbText = useMemo(() => PYQ, []);
            ...
            <div className="relative max-w-[1600px] mx-auto px-4 py-7 md:py-12">
               <div className="flex flex-col lg:flex-row lg:items-center gap-6 lg:gap-10">
                  <div className="flex-1 min-w-0">
                     <div className="inline-flex items-center gap-2 rounded-full border border-blue-200 bg-blue-50 px-3 py-1 text-[11px] font-extrabold text-blue-700">
                        <Sparkles size={14} /> Updated PYQ Collection
                     </div>
                     ...
                     <p className="mt-2 text-sm md:text-lg font-medium text-slate-600 max-w-3xl">
                        Select course & session to find the exact previous year question paper. This page is dedicated to PYQ only.
                     </p>
                        ...
                        <div className="rounded-2xl border border-gray-200 bg-white/90 backdrop-blur shadow-sm p-4">
                           <div className="flex items-center gap-2">
                              ...
                              <div className="text-xs">
                                 <div className="font-extrabold text-slate-900">Verified Uploads</div>
                     ...
                     <div className="rounded-2xl border border-gray-200 bg-white shadow-sm overflow-hidden">
                        <div className="p-4 bg-gradient-to-r from-blue-700 via-indigo-700 to-cyan-600 isp-shimmer text-white">
                           ...
                           <div className="mt-1 text-xs font-bold opacity-90">
                              Choose Course + Session to find the exact PYQ quickly.
                           </div>
                        </div>
                        <div className="p-5">
                           <div className="text-xs font-extrabold text-slate-900">Best way to find your PYQ</div>
               ...
               <div className="hidden md:block mt-6 rounded-2xl border border-gray-200 bg-white/80 backdrop-blur shadow-sm p-5">
                  <div className="text-base font-extrabold text-slate-900">
                     Download IGNOU Previous Year Question Papers (PYQ) – Session-wise
                  </div>
                  <div className="mt-1 text-sm font-semibold text-slate-600 leading-relaxed">
                     Choose your Course & Session to match the exact exam paper. If you want multiple categories (like Ebooks +
                     PYQ), select them and you’ll be redirected to All Products automatically.
                  </div>
                  <div className="mt-3">
                     <Link
                        href="/products"
            ...
            <div className="max-w-[1600px] mx-auto px-4">
               <div className="flex flex-col lg:flex-row gap-8 items-start relative">
                  ...
                  <div className="flex-1 w-full min-w-0">
                     ...
                     <div className="mt-5">
                        <ProductGrid selectedCat={[PYQ]} onMeta={setMeta} />
                     </div>
         ...
         <section className="bg-white border-t border-gray-100">
               ...
               <div className="rounded-2xl border border-gray-200 bg-gray-50 p-5 md:p-6 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                  <div className="min-w-0">
                     ...
                     <div className="mt-1 text-sm font-semibold text-slate-600">
                        Explore all categories in one place (Ebooks, Projects, Solved Assignments, Combo, etc). You can also combine filters there.
                     </div>
                  </div>
            ...
            <div className="max-w-[1600px] mx-auto px-4 py-12 md:py-16">
               ...
               <div className="grid md:grid-cols-3 gap-4">
                  {[
                     { t: "Updated PYQ Sets", d: "Session-wise papers for fast exam preparation." },
