...
export default function EbooksPage() {
  ...
  const [selectedCat, setSelectedCat] = useState<string[]>(
    urlCategory ? safeSplitComma(urlCategory) : [EBOOKS]
  );
  ...
  // ✅ keep this page URL only when category stays Ebooks-only
  const syncEbooksUrl = (nextCourse: string, nextSession: string, nextSearch: string) => {
    const params = new URLSearchParams(searchParams.toString());

    params.set("category", EBOOKS);
    ...
    const qs = params.toString();
    router.replace(`/ebooks${qs ? `?${qs}` : ""}`, { scroll: false });
  };

  // ✅ toggle category rule:
  // If user selects multiple categories OR any non-Ebooks => redirect to /products with same filters
  const handleToggleCategory = (cat: string) => {
    const current = Array.isArray(selectedCat) ? selectedCat : [EBOOKS];
    const next = current.includes(cat) ? current.filter((c) => c !== cat) : [...current, cat];
    const normalized = next.length === 0 ? [EBOOKS] : next;

    const hasOther = normalized.some((c) => c !== EBOOKS);
    ...
    setSelectedCat([EBOOKS]);
    syncEbooksUrl(selectedCourse, selectedSession, search);
  };
  ...
  useEffect(() => {
    ...
    const hasOther = c.some((x) => x !== EBOOKS);
  ...
  useEffect(() => {
    const t = setTimeout(() => {
      ...
      if (q === search) return;
      setSearch(q);
      syncEbooksUrl(selectedCourse, selectedSession, q);
    }, 500);
  ...
  const breadcrumbText = useMemo(() => EBOOKS, []);
    ...
    <main className="min-h-screen font-sans text-slate-800 bg-white">
      ...
      <div className="bg-white border-b border-gray-100">
        <div className="max-w-[1600px] mx-auto px-4 py-3 text-[13px] text-gray-500 flex items-center gap-2 overflow-x-auto whitespace-nowrap">
          ...
          <Link href="/ebooks" className="hover:text-blue-700 font-semibold">Ebooks</Link>
          <ChevronRight size={14} className="text-gray-300" />
        ...
        <div className="relative max-w-[1600px] mx-auto px-4 py-7 md:py-12">
          <div className="flex flex-col lg:flex-row lg:items-center gap-6 lg:gap-10">
            <div className="flex-1 min-w-0">
              <div className="inline-flex items-center gap-2 rounded-full border border-blue-200 bg-blue-50 px-3 py-1 text-[11px] font-extrabold text-blue-700">
                <Sparkles size={14} /> Exam-ready Ebooks & Notes
              </div>
              ...
              <p className="mt-2 text-sm md:text-lg font-medium text-slate-600 max-w-3xl">
                Select course & session to find the best ebook/notes. This page is dedicated to Ebooks only.
              </p>
              ...
              <div className="mt-5 md:hidden space-y-3">
                ...
                <div className="w-full rounded-2xl border border-gray-200 bg-white/90 backdrop-blur shadow-sm p-3">
                  <div className="text-[11px] font-extrabold text-slate-700 uppercase mb-2">Search ebooks</div>
                  ...
                  <div className="mt-2 text-[11px] text-slate-500 font-semibold">
                    Filters help you match the exact course & session.
                  </div>
                </div>
              </div>
            </div>
                ...
                <div className="rounded-2xl border border-gray-200 bg-white/90 backdrop-blur shadow-sm p-4">
                  <div className="flex items-center gap-2">
                    ...
                    <div className="text-xs">
                      <div className="font-extrabold text-slate-900">Exam Focused</div>
                      <div className="font-bold text-gray-500">Notes + guides</div>
                    </div>
                  </div>
                </div>
                <div className="rounded-2xl border border-gray-200 bg-white/90 backdrop-blur shadow-sm p-4">
              ...
              <div className="rounded-2xl border border-gray-200 bg-white shadow-sm overflow-hidden">
                <div className="p-4 bg-gradient-to-r from-blue-700 via-indigo-700 to-cyan-600 isp-shimmer text-white">
                  ...
                  <div className="mt-1 text-xs font-bold opacity-90">
                    Use Course + Session filters for perfect ebook matching.
                  </div>
                </div>
                <div className="p-5">
                  <div className="text-xs font-extrabold text-slate-900">Best way to find your ebook</div>
          ...
          <div className="hidden md:block mt-6 rounded-2xl border border-gray-200 bg-white/80 backdrop-blur shadow-sm p-5">
            <div className="text-base font-extrabold text-slate-900">
              Download IGNOU Ebooks & Notes (Course + Session-wise)
            </div>
            <div className="mt-1 text-sm font-semibold text-slate-600 leading-relaxed">
              Select Course & Session for best results. Multiple categories select karoge to All Products par auto-redirect ho jaoge.
            </div>
            <div className="mt-3">
              <Link
                href="/products"
        ...
        <div className="max-w-[1600px] mx-auto px-4">
          <div className="flex flex-col lg:flex-row gap-8 items-start relative">
            ...
            <div className="flex-1 w-full min-w-0">
              ...
              <div className="mt-5">
                <ProductGrid selectedCat={[EBOOKS]} onMeta={setMeta} />
              </div>
      ...
      <section className="bg-white border-t border-gray-100">
          ...
          <div className="rounded-2xl border border-gray-200 bg-gray-50 p-5 md:p-6 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
            <div className="min-w-0">
              ...
              <div className="mt-1 text-sm font-semibold text-slate-600">
                Explore all categories in one place (Solved Assignments, PYQ, Guess Papers, Projects, Combo, etc).
              </div>
            </div>
        ...
        <div className="max-w-[1600px] mx-auto px-4 py-12 md:py-16">
          ...
          <div className="grid md:grid-cols-3 gap-4">
            {[
              { t: "Exam-ready Content", d: "Notes, guides, and summaries for quick revision." },
              { t: "Instant PDF Download", d: "Download immediately after purchase (where applicable)." },
