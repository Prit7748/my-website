...
async function getInitialHandwrittenHardcopyData(input: {
  course?: string;
  session?: string;
  language?: string;
  search?: string;
...
export async function generateMetadata({ searchParams }: PageProps): Promise<Metadata> {
  ...
  const description = hasNonCanonicalParams
    ? `Browse IGNOU handwritten hardcopy delivery products${course ? ` for ${course}` : ""}${
        session ? ` (${session})` : ""
...
export default async function Page({ searchParams }: PageProps) {
  ...
  const { products, meta } = await getInitialHandwrittenHardcopyData({
  ...
  const initialQueryKey = buildHardcopyQueryKey({
    course: initialCourseParam,
    session: initialSessionParam,
    language: initialLanguageParam,
    search: initialSearchParam,
