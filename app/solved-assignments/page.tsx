...
function buildSolvedAssignmentsQueryKey(input: {
  ...
  if (safeStr(input.course)) params.set("course", safeStr(input.course));
  if (safeStr(input.session)) params.set("session", safeStr(input.session));
  if (safeStr(input.search)) params.set("search", safeStr(input.search));
  if (safeStr(input.sort)) params.set("sort", safeStr(input.sort));

  params.set("page", String(Math.max(1, safeNum(input.page, 1))));
  params.set("limit", "12");
...
export async function generateMetadata({ searchParams }: PageProps): Promise<Metadata> {
  ...
  const hasFilters = !!(course || session || search);

  const baseTitle = "Solved Assignments";
  const parts = [course, session].filter(Boolean);
  const dynamicTitle = parts.length ? `${baseTitle} - ${parts.join(" - ")}` : baseTitle;

  const description = hasFilters
    ? `Browse IGNOU solved assignments${course ? ` for ${course}` : ""}${session ? ` (${session})` : ""}${search ? ` matching "${search}"` : ""}.`
...
export default async function Page({ searchParams }: PageProps) {
  ...
  const initialSortParam = typeof sp.sort === "string" ? sp.sort : "latest";
  ...
  const initialQueryKey = buildSolvedAssignmentsQueryKey({
    ...
    session: initialSessionParam,
    search: initialSearchParam,
    sort: initialSortParam !== "latest" ? initialSortParam : "",
    page: pageNum,
  });
