...
export default function SolvedAssignmentsPage() {
  ...
  // ✅ Pagination/Sort real data ke liye meta state
  ...
  // ✅ URL se initial values
  const urlCategory = searchParams.get("category"); // comma separated
  const urlCourse = searchParams.get("course") || "";
  const urlSession = searchParams.get("session") || "";

  // ✅ DEFAULT SELECTED
  const [selectedCat, setSelectedCat] = useState<string[]>(
    urlCategory ? urlCategory.split(",").filter(Boolean) : ["Solved Assignments"]
  );

  // ✅ course/session state (chips ke liye)
  ...
  // ✅ state change → URL update (active pages)
  ...
  // ✅ URL manually change ho (back/forward) to state sync
  useEffect(() => {
    const c = searchParams.get("category");
    const course = searchParams.get("course") || "";
    const session = searchParams.get("session") || "";

    setSelectedCat(c ? c.split(",").filter(Boolean) : ["Solved Assignments"]);
    ...
    <main className="min-h-screen font-sans bg-white text-slate-800">
      ...
      <div className="h-[45px] bg-gray-50 border-b border-gray-200 flex items-center">
        <div className="max-w-[1600px] mx-auto px-4 w-full text-[14px] text-gray-500 whitespace-nowrap overflow-hidden text-ellipsis">
          ...
          /{" "}
          <Link href="/solved-assignments" className="hover:text-blue-600 mx-1">
            Solved Assignment
          </Link>{" "}
          /{" "}
          <span className="text-gray-900 font-medium ml-1 text-blue-700">
            {breadcrumbText}
          </span>
        </div>
      </div>
      ...
      <section className="bg-white py-10 border-b border-gray-100">
        <div className="max-w-[1600px] mx-auto px-4">
          ...
          <p className="text-gray-600 text-base md:text-lg leading-relaxed max-w-4xl">
            Get high-quality IGNOU study material. Select a category to browse specific products.
          </p>

          {/* ✅ URL-based active filter chips */}
          ...
          <div className="grid md:grid-cols-2 gap-8 text-gray-600 text-base leading-relaxed">
            <ul className="space-y-3">
              <li className="flex gap-3">
                <Check size={20} className="text-green-500 mt-1" /> 100% Correct Answers
              </li>
              <li className="flex gap-3">
                <Check size={20} className="text-green-500 mt-1" /> Instant PDF Download
              </li>
            </ul>
          </div>
        </div>
