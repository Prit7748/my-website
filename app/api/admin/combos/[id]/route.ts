...
function buildPyqComboThumb(args: {
  ...
  const params = new URLSearchParams();
  params.set("years", derivePyqYears(args.comboKind, args.itemCount));
...
function buildComboThumbUrl(args: {
  ...
  if (isPyqCategorySlug(args.categorySlug)) {
    return buildPyqComboThumb({
      ...
      itemCount: Array.isArray(args.itemsSnapshot) ? args.itemsSnapshot.length : 0,
