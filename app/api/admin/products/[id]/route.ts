...
async function syncCurrentProductStrong(productLike: any) {
  const productId = safeStr(productLike?._id);
  const productSku = safeStr(productLike?.sku);
  ...
  try {
    if (productSku) {
      return await syncProductAvailabilityBySku(productSku);
    }
  } catch (error: any) {
    return {
      ok: false,
      reason: safeStr(error?.message || "Availability sync failed"),
      productId,
      productSku,
    };
  }
  ...
  return {
    ok: false,
    reason: "Availability sync skipped",
    productId,
    productSku,
  };
}
  ...
  return NextResponse.json(
    {
      ...
      availabilityAutomation: {
        mode: "derived-and-synced-on-read",
        syncOk: Boolean(availabilitySync?.ok),
        syncReason: safeStr(availabilitySync?.reason || ""),
        finalAvailability:
          getAvailabilityAfterSync(availabilitySync) || safeStr(product?.availability || ""),
      },
    },
    { status: 200 }
  );
...
export async function PATCH(req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    let oldSkuSync: any = null;
    if (oldSku && oldSku !== safeStr(product.sku)) {
      oldSkuSync = await syncProductAvailabilityBySku(oldSku);
    }
    ...
    return NextResponse.json(
      {
        ...
        availabilityAutomation: {
          mode: "derived-and-synced-on-update",
          syncOk: Boolean(availabilitySync?.ok),
          syncReason: safeStr(availabilitySync?.reason || ""),
          finalAvailability:
    ...
    return NextResponse.json(
      {
        ...
        availabilityAutomation: {
          ...
          syncReason: safeStr(availabilitySync?.reason || ""),
          finalAvailability:
            getAvailabilityAfterSync(availabilitySync) || safeStr(restored?.availability || ""),
        },
      },
      { status: 200 }
    );
    ...
    return NextResponse.json(
      {
        ...
        availabilityAutomation: {
          ...
          syncReason: safeStr(availabilitySync?.reason || ""),
          finalAvailability:
            getAvailabilityAfterSync(availabilitySync) || safeStr(finalCreated?.availability || ""),
        },
      },
      { status: 201 }
    );
