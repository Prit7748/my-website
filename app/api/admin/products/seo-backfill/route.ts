...
function buildBaseFilter(category: string) {
    const filter: any = {
        isActive: true,
...
async function runBackfill(req: NextRequest, methodBody?: any) {
    ...
    const filter = buildBaseFilter(category);

    const candidates: ProductDoc[] = await Product.find(filter)
        .select(
            [
                "_id",
                "title",
    ...
    return NextResponse.json({
        ...
        skippedStrong,
        message: dryRun
            ? "SEO backfill preview completed. Execute only after checking preview."
            : "SEO backfill executed in bulk for this batch.",
        nextRecommendedUrl: `/api/admin/products/seo-backfill?dryRun=false&limit=${limit}${category ? `&category=${encodeURIComponent(normalizeCategory(category))}` : ""
            }`,
