// ✅ FILE: app/api/admin/courses/bulk/route.ts (COMPLETE REPLACE)
...
type Row = { code: string; title: string };
...
function aggregate(rows: Row[]) {
  ...
  for (const r of rows) {
    ...
    if (!code) continue;
    map.set(code, { code, title }); // last wins
  }
...
export async function POST(req: NextRequest) {
  ...
  const existing = await Course.find({ code: { $in: codes } }, { code: 1 }).lean();
  ...
  if (duplicates.length) {
    if (mode === "skip") {
      ...
      const ops = duplicates.map((x) => ({
        updateOne: {
          filter: { code: x.code },
          update: { $set: { ...(x.title ? { title: x.title } : {}) } },
          upsert: false,
        },
      }));
  ...
  return NextResponse.json(
    { dryRun: false, message: "Bulk upload applied", summary: { totalCodes: aggregated.length, created, updated, skipped } },
    { status: 200 }
  );
}
