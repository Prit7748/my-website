...
function normalizeAvailability(input: string) {
...
export async function POST(req: Request) {
  try {
    ...
    for (let i = 0; i < parsedRows.length; i++) {
      ...
      const dbAvailability = normalizeAvailability(row.H);

      const missingCourse: string[] = [];

      if (!sku || !subjectCodeRaw || !session || !language || !courseCodeRaw || !dbAvailability) {
        items.push({
          rowNumber,
          sku,
          status: "failed",
        ...
        items.push({
          ...
          reason: existing
            ? duplicateStrategy === "replace"
              ? `Duplicate mila: final create par existing product replace/update hoga. Pricing source: ${pricingResolution.source}`
              : `Duplicate mila: final create par new row ignore/skip hogi. Pricing source: ${pricingResolution.source}`
            : isDigitalForCategory
            ? `Ready to create as digital product. Pricing source: ${pricingResolution.source}`
            : `Ready to create as physical product. Pricing source: ${pricingResolution.source}`,
          courseCodes: courseCodeList,
          courseTitles,
        });
        continue;
          ...
          items.push({
            ...
            matchedBy:
              existing?.sku === sku && existing?.slug === normalizedSlugBase
                ...
                ? "sku"
                : "slug",
            reason: `Duplicate product already exists, new row ignored. Pricing source: ${pricingResolution.source}`,
            courseCodes: courseCodeList,
            courseTitles,
          });
          skippedRows++;
        ...
        items.push({
          ...
          matchedBy:
            existing?.sku === sku && existing?.slug === normalizedSlugBase
              ...
              ? "sku"
              : "slug",
          reason: `Existing product replaced successfully. Pricing source: ${pricingResolution.source}`,
          courseCodes: courseCodeList,
          courseTitles,
        });
        updatedRows++;
      ...
      items.push({
        ...
        slug: finalSlug,
        status: "created",
        reason: `Created successfully. Pricing source: ${pricingResolution.source}`,
        courseCodes: courseCodeList,
        courseTitles,
      });
      createdRows++;
