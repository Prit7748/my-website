...
import { processBulkImagesJobBatch } from "@/lib/bulkProductImagesJob";
...
export async function POST(req: Request) {
  ...
  if (safeStr(currentJob?.jobType) !== "product_images") {
    return NextResponse.json(
      { ok: false, error: "This route only processes product_images jobs" },
      { status: 400 }
    );
  }
  ...
  try {
    const batchResult = await processBulkImagesJobBatch({
      job: lockedJob,
      batchNumber,
      fromIndex,
      toIndex,
    ...
    const failedJob = await failBulkUploadJob({
      ...
      createdBy,
      lockToken: claim.lockToken,
      message: safeStr(error?.message || "Bulk product images batch processing failed"),
    });

    return NextResponse.json(
      {
        ok: false,
        error: safeStr(error?.message || "Bulk product images batch processing failed"),
