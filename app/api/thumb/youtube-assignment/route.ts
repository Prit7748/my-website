...
function buildTokenMapFromSearchParams(searchParams: URLSearchParams) {
  ...
  return {
    ...
    "%5": normalizeSpaces(searchParams.get("session")) || "July 2025",
    "%6": normalizeSpaces(searchParams.get("medium")) || "Hindi",
    "%7": normalizeSpaces(searchParams.get("productLink")) || "",
    "%8": normalizeSpaces(searchParams.get("productTitle")) || "",
    "%9": normalizeSpaces(searchParams.get("category")) || "Solved Assignments",
...
function fallbackBackgroundSvg() {
  ...
  <rect x="68" y="72" width="290" height="56" rx="28" fill="#facc15" filter="url(#softShadow)"/>
  <text x="213" y="100" text-anchor="middle" dominant-baseline="middle" font-family="Arial Black, Arial, sans-serif" font-size="24" font-weight="900" fill="#111827">SOLVED ASSIGNMENT</text>
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    templateImageUrl = safeStr(config?.assignmentThumbnail?.templateImageUrl);
    fields = Array.isArray(config?.assignmentThumbnail?.fields)
      ? config.assignmentThumbnail.fields
      : [];
  } catch {
    templateImageUrl = "";
    fields = [];
