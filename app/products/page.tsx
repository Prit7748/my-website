...
function buildProductsQueryKey(input: {
  ...
  const selectedCat = uniqueStrings(parseList(input.category));
  ...
  const sort = (safeStr(input.sort) as SortKey) || "latest";
...
async function getInitialProductsData(input: {
  category?: string;
  course?: string;
  session?: string;
  language?: string;
  ...
  const categories = uniqueStrings(parseList(input.category));
...
export default async function Page({ searchParams }: PageProps) {
  ...
  const initialCategoryParam = typeof sp.category === "string" ? sp.category : "";
  ...
  const { products, meta } = await getInitialProductsData({
