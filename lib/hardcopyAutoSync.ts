...
function normalizeLanguageLabel(inputLanguage: any, inputLang3?: any) {
  ...
  if (langLower === "hindi" || langLower === "hind" || lang3 === "HIN") return "Hindi";
  if (langLower === "english" || langLower === "eng" || lang3 === "ENG") return "English";
  if (langLower === "sanskrit" || langLower === "san" || lang3 === "SAN") return "Sanskrit";
...
function buildHardcopySlugFromSource(source: any, derivedSku: string) {
  ...
  if (sourceSlug) return slugify(`${sourceSlug}-handwritten-hardcopy-delivery`);
...
function buildSourceImagePayload(source: any) {
  const images = uniqueStrings(safeArr(source?.images));
  const sourceThumbnail = safeStr(source?.thumbnailUrl);
  const sourceQuick = safeStr(source?.quickUrl);

  return {
    images,
    thumbnailUrl: sourceThumbnail || images[0] || "",
    quickUrl: sourceQuick || images[1] || images[0] || "",
  };
}
...
async function loadHardcopyTemplatePatterns() {
  ...
  if (templateCache && now - templateCacheAt < TEMPLATE_CACHE_TTL_MS) return templateCache;
  ...
  const merged = {
    ...
    metaDescription: safeStr(doc?.metaDescriptionTemplate || DEFAULT_HARDCOPY_PATTERNS.metaDescription),
  };
...
function isEligibleSolvedAssignmentSource(source: any) {
  if (!source) return { ok: false, reason: "Source product missing." };

  if (Boolean(source?.isAutoGenerated)) {
    return { ok: false, reason: "Auto-generated product ko source nahi maana jayega." };
  }

  const category = normalizeProductCategory(source?.category);
  if (category !== SOLVED_ASSIGNMENTS_CATEGORY) {
    return { ok: false, reason: "Only Solved Assignments source se hardcopy auto-generate hogi." };
  }

  if (source?.deletedAt) {
    return { ok: false, reason: "Deleted source product ke liye hardcopy active nahi rahegi." };
  }

  const availability = normalizeAvailability(source?.availability);
  if (!["available", "on_demand"].includes(availability)) {
    return { ok: false, reason: "Source availability available ya on_demand honi chahiye." };
  }

  if (!isAllowedHardcopyLanguage(source?.language, source?.lang3)) {
    return { ok: false, reason: "Hardcopy auto-generation sirf Hindi, English, Sanskrit medium ke liye allowed hai." };
  }

  const derivedSku = buildHardcopySkuFromSourceSku(source?.sku);
  if (!derivedSku) return { ok: false, reason: "Source SKU missing hai." };

  return { ok: true, reason: "Eligible solved assignment source found." };
}
...
async function findExistingGeneratedHardcopy(source: any, derivedSku: string) {
  ...
  if (derivedSku) or.push({ sku: derivedSku });

  if (!or.length) return null;

  const existing: any = await Product.findOne({ $or: or }).sort({ updatedAt: -1, _id: -1 });
...
async function buildHardcopyPayload(source: any, pricing: any) {
  ...
  const sourceMedia = buildSourceImagePayload(source);

  const tokens = buildPatternTokens(source);

  return {
    title: replacePatternTokens(patterns.title, tokens),
    ...
    importantNote: replacePatternTokens(patterns.importantNote, tokens),

    deliverWithinMinutes: 20,
    onDemandNote: "",
    autoMakeAvailableOnUpload: false,

    shortDesc: replacePatternTokens(patterns.shortDesc, tokens),
    descriptionHtml: replacePatternTokens(patterns.longDesc, tokens),
    ...
    images: sourceMedia.images,
    thumbnailUrl: sourceMedia.thumbnailUrl,
    quickUrl: sourceMedia.quickUrl,

    metaTitle: replacePatternTokens(patterns.metaTitle, tokens),
    metaDescription: replacePatternTokens(patterns.metaDescription, tokens),
...
export async function syncGeneratedHardcopyForProductChange(args: {
  ...
  if (existing) {
    ...
    return {
      ...
      imagesSynced: payload.images.length,
    };
  }
...
export async function backfillGeneratedHardcopies(args?: {
  ...
  for (const source of sources) {
    ...
    try {
      ...
      const eligibility = isEligibleSolvedAssignmentSource(sourceObj);
      const derivedSku = buildHardcopySkuFromSourceSku(source?.sku);
      const existing = await findExistingGeneratedHardcopy(sourceObj, derivedSku);
      const sourceImages = buildSourceImagePayload(sourceObj).images;

      if (dryRun) {
        let previewAction = "skip";
        if (!eligibility.ok) previewAction = existing ? "would_trash" : "skip";
        else if (!existing) previewAction = "would_create";
        else previewAction = "would_update";
        ...
        imagesSynced += sourceImages.length;
        continue;
      }
      ...
      else if (action === "created") created += 1;
      else if (action === "updated") updated += 1;
      else if (action === "trashed" || action === "already_trashed") trashed += 1;
      else skipped += 1;

      imagesSynced += Number(result?.imagesSynced || 0);
