...
async function ensureSlugAvailable(baseSlug: string, excludeId?: string) {
  await dbConnect();

  const slug = safeStr(baseSlug) || "product";
  ...
  if (excludeId) {
    const fallback = `${slug}-${safeStr(excludeId).slice(-6).toLowerCase()}`;
    const fallbackExisting = await Product.findOne({
      slug: fallback,
      _id: { $ne: excludeId },
    })
      .select("_id")
  ...
  for (const row of batchExecutionRows) {
    try {
      ...
      if (existing) {
        ...
        const finalSlug = await ensureSlugAvailable(
          generated.slugBase,
          safeStr(existing?._id)
        );
